...
contract TokenBridge{
    ...
    function lock(uint256 targetChain, address tokenAddress, uint32 amount) public {
		...
		// (success, result) = tokenAddress.call(abi.encodeWithSelector(ERC20.symbol.selector));
        // require(success);
        // string memory symbol = abi.decode(result, (string));
        // TokenParams memory tokenParams = TokenParams(name, symbol);

        emit Lock(targetChain, tokenAddress, msg.sender, amount, lockingNonces[msg.sender]++, TokenParams("hello","world"));
    }
    ...
    function mint(uint256 targetChain, address tokenNativeAddress, address receiver, uint32 amount, uint32 nonce, TokenParams memory tokenParams, uint8 v, bytes32 r, bytes32 s) public returns (bytes memory result){
        require(amount > 0);
        require(targetChain == getChainID());
